object frmConfiguracaoInbox: TfrmConfiguracaoInbox
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'Formulario de Configuracao Inbox'
  ClientHeight = 448
  ClientWidth = 819
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnl1: TPanel
    Left = 0
    Top = 399
    Width = 819
    Height = 49
    Align = alBottom
    TabOrder = 0
    object cxButton1: TcxButton
      AlignWithMargins = True
      Left = 698
      Top = 4
      Width = 117
      Height = 41
      Align = alRight
      Caption = 'Fechar'
      OptionsImage.Glyph.Data = {
        
        B5D040437D951212223000000004000000010000000000000000}
      TabOrder = 0
      OnClick = cxButton1Click
    end
  end
  object cxGroupBox1: TcxGroupBox
    AlignWithMargins = True
    Left = 3
    Top = 3
    Align = alClient
    Caption = 'Configura'#231#245'es'
    TabOrder = 1
    Height = 393
    Width = 813
    object btnClienteVarejoAtacado: TcxButton
      Left = 40
      Top = 93
      Width = 243
      Height = 49
      Caption = 'Criar cliente padr'#227'o atacado/varejo'
      OptionsImage.Glyph.Data = {
        
        56B00E3226560000000600000002000000000000000000000000}
      TabOrder = 0
      OnClick = btnClienteVarejoAtacadoClick
    end
    object cxLabel1: TcxLabel
      Left = 495
      Top = 119
      Caption = 'Cria cliente padr'#227'o para atacado / varejo'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
      Transparent = True
    end
    object btnViewsMercafacil: TcxButton
      Left = 40
      Top = 162
      Width = 243
      Height = 49
      Caption = 'Cria Views MercaF'#225'cil'
      OptionsImage.Glyph.Data = {
        
        56B00E3226560000000600000002000000000000000000000000}
      TabOrder = 2
      OnClick = btnViewsMercafacilClick
    end
    object cxLabel2: TcxLabel
      Left = 295
      Top = 175
      Caption = 'Cria as views necess'#225'rias para o funcionamento mercaf'#225'cil'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
      Transparent = True
    end
    object btnBloqueioOfertaRegiao: TcxButton
      Left = 40
      Top = 226
      Width = 243
      Height = 49
      Caption = 'TRIGGER BLOQUEIO OFERTA - REGIAO'
      OptionsImage.Glyph.Data = {
        
        56B00E3226560000000600000002000000000000000000000000}
      TabOrder = 4
      OnClick = btnBloqueioOfertaRegiaoClick
    end
    object txtInicial: TcxTextEdit
      Left = 295
      Top = 38
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -16
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.IsFontAssigned = True
      TabOrder = 5
      Text = '200000'
      Width = 90
    end
    object cxLabel5: TcxLabel
      Left = 295
      Top = 93
      Caption = 'Cli. Varejo'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
      Transparent = True
    end
    object cxLabel6: TcxLabel
      Left = 391
      Top = 93
      Caption = 'Cli. Atacado'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
      Transparent = True
    end
    object btnBloqueioOfertaEmbalagem: TcxButton
      Left = 40
      Top = 290
      Width = 243
      Height = 49
      Caption = 'TRIGGER BLOQUEIO OFERTA - EMBALAGEM'
      OptionsImage.Glyph.Data = {
        
        56B00E3226560000000600000002000000000000000000000000}
      TabOrder = 8
      OnClick = btnBloqueioOfertaEmbalagemClick
    end
    object txtClivarejo: TcxCurrencyEdit
      Left = 295
      Top = 114
      AutoSize = False
      Properties.DisplayFormat = ',0.00;- ,0.00'
      Properties.ReadOnly = True
      TabOrder = 9
      Height = 25
      Width = 81
    end
    object txtCliatacado: TcxCurrencyEdit
      Left = 391
      Top = 114
      AutoSize = False
      Properties.DisplayFormat = ',0.00;- ,0.00'
      Properties.ReadOnly = True
      TabOrder = 10
      Height = 25
      Width = 81
    end
  end
  object btnSequenceCliente: TcxButton
    Left = 43
    Top = 39
    Width = 243
    Height = 49
    Caption = 'Cria sequence cad. clientes'
    OptionsImage.Glyph.Data = {
      
      56B00E3226560000000600000002000000000000000000000000}
    TabOrder = 2
    OnClick = btnSequenceClienteClick
  end
  object cxLabel4: TcxLabel
    Left = 394
    Top = 52
    Caption = 'Cria sequence de codcli.'
    ParentFont = False
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clWindowText
    Style.Font.Height = -13
    Style.Font.Name = 'Tahoma'
    Style.Font.Style = [fsBold]
    Style.IsFontAssigned = True
    Transparent = True
  end
end
